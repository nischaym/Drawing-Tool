d2.directive('draw2dCanvas', ['$window','$parse', '$timeout', ($window,$parse, $timeout) => {
       ...
       return {
    		   restrict: 'E,A',
   	           link: (scope, element, attrs, controller) => {
	   	           ...
	   	           let canvas = new draw2d.Canvas(element.attr('id'), scope.editor.canvas.width, scope.editor.canvas.height);
    	           ...
    	           // CommandStack
    	           let stack = canvas.getCommandStack();
    	           stack.addEventListener((event) => {
    	               $timeout(() => {
    	           ...
    	           let changeCallback = function(emitter, attribute){
    	        	   $timeout(() =>{
    	           ...
    	           canvas.on('select', (canvas, figure) => {
    	               $timeout(() => {
    	            	   ...
    	            	   //
    	            	   if(scope.editor.selection.figure !== null) {
	               ...
	               scope.$watchCollection('editor.selection.attr', (newValues, oldValues) => {
    	               ...
    	               if(oldValues !== null && scope.editor.selection.figure != null) {
    	            	   ...
    	            	   let changes = draw2d.util.JSON.diff(newValues, oldValues);
    	           ...
    	           scope.editor['delete'] = $.proxy(function() {
    	   			  let node = this.selection.primary;
